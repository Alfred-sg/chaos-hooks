...
const useForm = (options?: Options) => {
  const { onValuesChange } = options || {} as Options;

  const fieldsMeta = useRef<{ [name: string]: FieldMeta }>({});// 平铺结构
   ...
   * @param value 
   */
  const setFieldMeta = useCallback((name: string, key: string | FieldMeta, value?: any) => {
    if (typeof key !== 'string'){
      fieldsMeta.current[name] = key || {};
    } else if (typeof key === 'string'){
      if ( !fieldsMeta.current[name] ) fieldsMeta.current[name] = {};
      const meta: FieldMeta = fieldsMeta.current[name];
      meta[key] = value;
    };
  }, [fieldsMeta]);
   ...
   * @param key 
   */
  const getFieldMeta = useCallback((name: string, key?: string) => {
   ...
   * @param meta 
   */
  const getFieldDecorator = useCallback((name: string, meta: FieldMeta = {} as FieldMeta) => {
    ...
    return (inst: React.ReactElement) => {
      ...
      return (
        <VirtualField 
          unRegisterField={() => { unRegisterField(name) }}
        >
          {React.cloneElement(inst, props)}
        </VirtualField>
      );
    }
  }, [fieldsMeta, fields, fieldsRef]);
   ...
   * @param name 
   */
  const getFieldRef = useCallback((name: string) => {
   ...
   * @param name 
   */
  const getFieldInitialValue = useCallback((name: string) => {
   ...
   * @param vals 
   */
  const setFieldsValue = useCallback((vals: Fields) => {
    getFieldsName().forEach(name => {
